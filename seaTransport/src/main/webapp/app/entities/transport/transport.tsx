...
import { getEntities, reset } from './transport.reducer';
...
export const Transport = (props: ITransportProps) => {
  ...
  const { transportList, match, loading } = props;
  return (
    <div>
            ...
            <Table responsive>
              <thead>
                <tr>
                  ...
                  <th className="hand" onClick={sort('transportName')}>
                    Transport Name <FontAwesomeIcon icon="sort" />
                  </th>
                  <th className="hand" onClick={sort('maxWeight')}>
                    Max Weight <FontAwesomeIcon icon="sort" />
                  </th>
                  <th className="hand" onClick={sort('speed')}>
                    Speed <FontAwesomeIcon icon="sort" />
                  </th>
                  <th className="hand" onClick={sort('deckSize')}>
                    Deck Size <FontAwesomeIcon icon="sort" />
                  </th>
                  <th />
                </tr>
              </thead>
              <tbody>
                {transportList.map((transport, i) => (
                  <tr key={`entity-${i}`} data-cy="entityTable">
                    ...
                    <td>{transport.maxWeight}</td>
                    <td>{transport.speed}</td>
                    <td>{transport.deckSize}</td>
...
const mapStateToProps = ({ transport }: IRootState) => ({
  ...
  totalItems: transport.totalItems,
  links: transport.links,
  entity: transport.entity,
  updateSuccess: transport.updateSuccess,
});
